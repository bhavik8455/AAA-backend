...
app.get('/student/tasks/:status', async (c) => {
	...
	const studentTasks = await db
		...
		.where(and(eq(submissions.studentId, studentId as string), eq(submissions.status, status)))
		.groupBy(
			tasks.id,
			tasks.title,
			tasks.taskType,
...
app.post('/student/submission/upload', async (c) => {
	...
	await db
		...
		.where(and(eq(submissions.taskId, body['taskId'].toString()), eq(submissions.studentId, body['studentId'].toString())));
...
app.post('/teacher/addTask', async (c) => {
	...
	try {
		...
		if (!body.teacherSubjectId || !body.semester || !body.taskType || !body.title || !body.dueDate || !body.totalMarks || !body.division) {
			return c.json(
				{
					success: false,
		...
		const studentsInClass = await db
			...
			.from(students)
			.where(and(eq(students.currentSemester, body.semester), eq(students.division, body.division)));
...
app.get('/teacher/tasks', async (c) => {
	...
	const tasksList = await db
		...
		.from(tasks)
		.innerJoin(teacherSubjects, eq(tasks.teacherSubjectId, teacherSubjects.id))
		.where(and(eq(tasks.semester, parseInt(semester)), eq(teacherSubjects.subjectId, subjectId), eq(teacherSubjects.division, division)))
...
app.get('/teacher/students-list', async (c) => {
	...
	const studentsList = await db
		...
		.leftJoin(submissions, eq(submissions.studentId, students.id))
		.leftJoin(marks, eq(marks.submissionId, submissions.id))
		.where(and(eq(students.currentSemester, parseInt(semester)), eq(students.division, division), eq(submissions.taskId, taskId)))
		.groupBy(students.rollNumber, users.fullName, submissions.status, submissions.submissionDate, submissions.submissionFilePath)
		.orderBy(students.rollNumber);
...
app.post('/teacher/save-marks', async (c) => {
	...
	try {
		...
		for (const mark of marksArray) {
			...
			const submission = await db.select().from(submissions).where(eq(submissions.id, mark.submissionId)).limit(1);
			...
			const teacher = await db.select().from(teachers).where(eq(teachers.id, mark.markedBy)).limit(1);
...
app.post('/auth/login', async (c) => {
	...
	try {
		...
		if (role === 'student') {
			const studentDetails = await db.select().from(students).where(eq(students.userId, user.id)).limit(1);

			additionalDetails = studentDetails[0] || null;
		} else if (role === 'teacher') {
			const teacherDetails = await db.select().from(teachers).where(eq(teachers.userId, user.id)).limit(1);
	...
	try {
		...
		for (let i = 1; i < rows.length; i++) {
			...
			try {
				...
				const existingPid = await db.select().from(students).where(eq(students.pid, pid)).limit(1);
...
app.post('/subjects', async (c) => {
	...
	try {
		...
		const existingSubject = await db.select().from(subjects).where(eq(subjects.subjectCode, subjectCode)).limit(1);
...
app.delete('/teacher/task/:taskId', async (c) => {
	...
	try {
		...
		const submissions_list = await db.select().from(submissions).where(eq(submissions.taskId, taskId));

		// 2. Delete marks for each submission
		for (const submission of submissions_list) {
			await db.delete(marks).where(eq(marks.submissionId, submission.id));
		}
